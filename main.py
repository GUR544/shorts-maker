...
def local_css():
    ...
    <style>
        ...
        body, .stApp { font-family: 'Inter', sans-serif; background: #121212; color: #ffffff; }
        #MainMenu, .stDeployButton, footer { visibility: hidden; }
        header { visibility: hidden; height: 0; margin: 0; }
        .main-container { max-width: 800px; margin: 2rem auto; padding: 2.5rem; background: rgba(30, 30, 30, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 20px; box-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.37); backdrop-filter: blur(10px); }
        h1 { font-size: 3rem; font-weight: 900; text-align: center; background: linear-gradient(90deg, #8A2BE2, #4B0082, #FF6B6B); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        .st-emotion-cache-10trblm, .st-emotion-cache-1629p8f h2 { text-align: center; color: #b3b3b3; font-weight: 400; }
    </style>
    """
...
def download_video(_url, cookie_file=None):
    """Downloads video, preferring H.264 (AVC) codec to avoid AV1 issues."""
...
def create_short_with_ffmpeg(video_path, start, end, aspect_ratio, fps, clip_index):
    ...
    if aspect_ratio == "9:16":
        filter_complex = (
            f"[0:v]split[main][bg];"
            f"[bg]crop=ih*9/16:ih,scale=1080:1920,boxblur=luma_radius=10:luma_power=1[bg_blurred];"
            f"[main]scale=1080:-2[fg];"
            f"[bg_blurred][fg]overlay=(W-w)/2:(H-h)/2,fps={fps}"
        )
    ...
    try:
        ...
        st.error(f"FFMPEG Error creating clip {clip_index}: {e}")
        return None
...
# ... (UI for options and preview pages) ...
