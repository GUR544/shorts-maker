...
def local_css():
    ...
    <style>
        /* General Styling */
        @import url('https://fonts.googleapis.comcom/css2?family=Inter:wght@400;600;700;900&display=swap');
        body, .stApp {
        ...
        /* Helper text for cookie uploader */
        .st-emotion-cache-1b2dsde { 
            font-size: 0.9rem; 
            color: #b3b3b3;
        }
...
def get_video_info(url, cookie_file=None):
    """Fetches video info, using cookies if provided."""
...
def download_video(_url, cookie_file=None):
    """Downloads video, using cookies if provided."""
...
if st.session_state.page == "landing":
    ...
    st.markdown(
        "**Why?** For age-restricted videos or to avoid 'bot' errors, "
        "uploading your YouTube cookies allows this app to download on your behalf. "
        "Use an extension like 'Get cookies.txt' to export your cookies.", 
...
# --- PROCESSING & PREVIEW WINDOW ---
    ...
    if not st.session_state.generated_clips:
        with st.spinner("Downloading, analyzing, and rendering clips... This may take a few minutes."):
            ...
            for i, (start, end) in enumerate(highlights):
                clip_path = create_short_with_ffmpeg(
                    video_path, start, end, st.session_state.aspect_ratio, st.session_state.fps, i
                )
